...
검증일: 2026-09-07. MariaDB 11.4, macOS / Python 3.12. 운영 DB와 사용자의 데모 DB는 변경하지 않고 별도 Docker Source/Target(33316/33318)에서 검증했습니다. 대상 테이블은 `snapshot_date` 선두 인덱스가 없으면 사전 검사에서 거부합니다. 최종 반영 트랜잭션 크기와 대상 DB 자원 상한은 여전히 운영 환경에서 별도 관리해야 합니다.

## 확인한 위험과 방어 범위

| 항목 | 현재 동작 / 영향 | 검증 결과 및 보강 방향 |
| --- | --- | --- |
| 기존 대상의 날짜 인덱스 누락 | 날짜 교체와 비교 조회가 전체 스캔할 수 있음 | `snapshot_date` 선두 인덱스가 없으면 대상 사전 검사에서 중단 |
...
테스트는 연결한 두 서버의 `snapshot_source`와 `snapshot_target`을 삭제·재생성합니다. 반드시 별도 폐기용 서버만 지정하세요. 실제 환경의 CPU/I/O, 기존 업무 쿼리 응답시간, 복제 지연, 최대 행 크기, 디스크·로그 사용량의 허용치를 이 소규모 시험으로 보장하지 않습니다.

운영 적용 전에는 대상 잠금·쿼리 시간 제한과 적재 속도·전체 반영 크기를 별도로 관리하세요. 최종 반영을 단순히 여러 번 커밋하면 현재의 테이블 묶음 원자성이 깨지므로 별도 설계가 필요합니다.
