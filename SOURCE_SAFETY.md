...
2026-09-08 / v0.2.9. 설치 경로를 제외한 원본 DB 접근 경로를 다시 점검했습니다. 읽기 전용 계정이 아닌 경우를 포함해, 앱이 원본에 보내는 명령·조회·대기·재시도·대상 우회 경로를 점검했습니다. **읽기가 원본에 잠금이나 부하를 전혀 주지 않는다는 보장은 불가능합니다.** 아래 변경은 위험한 실행 경로를 제거하고 남는 조회에 보수적인 한계를 적용합니다. 운영 DB에는 접속하지 않았습니다.
