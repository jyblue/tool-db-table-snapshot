...
2026-09-08 / v0.2.10. 설치 경로를 제외한 원본 DB 접근 경로를 다시 점검했습니다. 읽기 전용 계정이 아닌 경우를 포함해, 앱이 원본에 보내는 명령·조회·대기·재시도·대상 우회 경로를 점검했습니다. **읽기가 원본에 잠금이나 부하를 전혀 주지 않는다는 보장은 불가능합니다.** 아래 변경은 위험한 실행 경로를 제거하고 남는 조회에 보수적인 한계를 적용합니다. 운영 DB에는 접속하지 않았습니다.
...
| 연결 테스트·날짜별 비교 | 연결 확인은 서버 정보·테이블 목록만 조회. 날짜별 비교도 등록된 모든 Source와 Target 격리를 **대상 schema 선택 전에** 확인하고 Target 읽기 연결에 같은 세션 제한을 적용. 부분 결과 경고도 실패 처리 |
...
서로 다른 폐기용 MariaDB 11.4 컨테이너(33316 / 33318)에서 검증했습니다. 사용자 데모 데이터와 운영 DB는 테스트 대상으로 사용하지 않았습니다.
